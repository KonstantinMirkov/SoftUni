...
public class Bomb {
    ...
    public static void main(String[] args) {
        ...
        Deque<String> directions = Arrays.stream(scanner.nextLine().split(",")).collect(Collectors.toCollection(ArrayDeque::new));

        char[][] matrix = creatingMatrix(scanner, sizeOfMatrix);
    ...
    private static char[][] creatingMatrix(Scanner scanner, int size) {
        ...
        for (int row = 0; row < size; row++) {
            ...
            matrix[row] = word.toString().toCharArray();
            if (word.toString().contains("s")) {
                playerRow = row;
